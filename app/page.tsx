...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("/api/decisions")
      ...
      .catch(() => setError("Unable to load decisions."))
      .finally(() => setLoading(false));
  }, []);

  if (loading) {
    return (
      <main className="p-10 text-xl font-semibold">
        Loading decisions...
      </main>
    );
  }
  ...
  if (error) {
    return (
      <main className="p-10 text-red-700 font-semibold">
        {error}
      </main>
    );
  }
  ...
  if (data.length === 0) {
    return (
      <main className="p-10 text-gray-700 font-semibold">
